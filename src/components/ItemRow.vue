<template>
    <div class="flex flex-row hover:text-slate-50">
        ...
        >
            <div
                role="status"
                class="flex justify-center h-full pt-3"
            >
        ...
        <div v-else class="basis-3/4">
            <div :class="{'flex': true, 'bg-slate-800': mode === 'edit'}">
                ...
                <input 
                    ...
                    v-model="record.name"
                />
                <div class="flex-auto w-1/4 p-3" v-if="mode !== 'edit'">{{ record.amount }}</div>
                <input 
                    v-else
...
<script>
    export default {
        ...
        methods: {
            editItem() {
               this.mode = 'edit';
               this.initialAmount = this.record.amount;
               this.initialname = this.record.name;
            },
            saveItem() {
                this.mode = 'normal';

                if (this._isChanged()) {
                    this.$emit('save-item', this.record, this.loaderCallback);
                }
            ...
            removeItem() {
                this.$emit('remove-item', this.record, this.loaderCallback);
            },
            loaderCallback(value) {
                this.isLoading = value;
            },
            _isChanged() {
                return this.initialAmount !== this.record.amount || this.initialname !== this.record.name;
            }
        },
    }
</script>
